...
- **Journey playback on the phone — new `ProximiioJourney` module, re-exported
  by the umbrella.** `Proximiio.fetchJourney(id:)` reads a journey drawn in
  MapTap from `v7/geo/journeys/{id}` with the SDK's own token and validates it
  (`ProximiioJourneyError.invalidID` before any request, `.unplayable` with the
  reason); `ProximiioJourney` decodes the document as leniently as the geo API
  stores it and ports LiveView's playability gate. `ProximiioJourneyTimeline`
  ports LiveView's Rust timeline (dwells, per-leg speeds, floor changes that
  last at least `transition_s` and switch level on arrival) and is checked
  against output generated by the Rust code: legs, total duration to the bit,
  level-switch times and positions at every sampled time.
  `JourneyPlaybackProvider` is a `CustomPositionProviding` that plays the
  timeline at `JourneyPlaybackConfiguration.rate` (1–10 Hz, default 4) and
  `speed` (0.5–10×), with `loops`, `startOffset`, `pausePlayback()` /
  `resumePlayback()` (a pause keeps emitting the held position) and LiveView's
  12 h run cap. Each frame carries the Proximi.io floor resolved from the
  waypoint level against the synced floors; a level with no floor is emitted
  without one, listed in `Diagnostics.unmappedLevels` and logged once. No
  relay and no network beyond the fetch. Time goes through
  `JourneyPlaybackClock` / `JourneyPlaybackSleeping`, so tests run with no
  wall-clock waits.
- **`Proximiio.journeys()` lists the organisation's journeys** from
  `v7/geo/journeys` with the SDK's token, in the API's order. The list is not
  validated: every journey the API returns is in it, so a picker can show an
  unplayable one next to its `validationFailure()` reason instead of hiding it.
  A body that is not a JSON array of journey objects is
  `ProximiioAPIError.decoding`.
- **`ProximiioDevices` manages DL-TDoA anchors — the third firmware line
  (`protocol.md §13`).** `PRXDeviceName.Line.dlTDoA` parses `PRX-DL-<I|R|T><id>`
  (decimal id 1–255, role letter in `dlRole`); a lean `0x0100` anchor without
  the OOB characteristics is classified by its name (or by `0x0109`), and the
  fleet scan now includes `0x0100` so DL anchors appear at all. New codecs:
  `PRXDLConfigRecord` (`0x0003` with the DL role in the high nibble and
  3 × int16 cm coordinates — `readConfig()` and `readDLConfig()` each refuse the
  other line's unit, so neither layout is ever misread as the other),
  `PRXDLProvisioning` / `PRXDLConfigDraft` / `PRXDLCommitPlan` (lean
  `[field][value]` writes; stop → commit → start only for schedule fields, or
  for every commit on fw 0.1.0; a no-op save writes nothing),
  `PRXDLControl` (`0x0202` stop / start / enter-DFU / identify — no interval),
  `PRXAntennaDelay` (`0x0107`, 2-byte form) and `PRXDLHealth` (`0x0109`,
  fw 0.2.0+). `PRXDeviceSession` gains `readDLConfig()`, `readDLHealth()`,
  `provisionDL(_:)` (refusals surface as `PRXDLProvisioningError` with step and
  ATT code), `restoreDLDefaults()`, `identifyDL(seconds:)`, `stop/startDLSchedule()`
  and the antenna-delay read/write; `PRXDeviceFeature` gains `.dlHealth`,
  `.dlLED`, `.dlLiveCommit` and `.antennaDelay`. `PRXDFUEngine` updates a DL
  anchor by stopping its schedule before `cmd 2`, refuses a unit whose `0x0106`
  byte 7 bit 0 is clear (wire-only), and after `os reset` verifies the unit came
  back with the same id, role and session (`PRXDFUFailure.identityChanged`).
  `PRXDLCell` validates a cell (one initiator, unique ids and slots, responder
  list = responders present, ≤ 6 responders) and orders a rollout responders
  first, initiator last. `PRXSimulatedDevice.dlAnchor(…)` / `dlBenchCell()`
  simulate the firmware's answers, refusals included.
- **DL-TDoA fw 0.3.0 beacons and fw 0.4.x LED (`protocol.md §13.12`, §13.4 `0x16`).**
  `PRXDLBeaconConfig` reads `0x010A` (44 B) and carries the auto-identity rules
  (major = session & 0xFFFF, minor = id, instance = session BE32 ‖ 00 ‖ id);
  `PRXDLBeaconFlags`; provisioning fields `0x03`–`0x07` and `0x17`–`0x1B`, all
  live; `PRXDLConfigRecord.ibeacon` (effective iBeacon fields at the §2.2
  offsets); `PRXDLConfigDraft` diffs the beacon layer and never writes a
  major/minor or instance while its auto flag is on. `PRXDLLEDMode.pulse` (2),
  gated by `PRXDeviceFeature.dlLEDPulse`; `.dlBeacons`. `PRXDLCell` flags shared
  LED colours as a non-blocking issue and suggests the bench palette. The
  simulator defaults to fw 0.4.1 and `dlBenchCell()` is now I1 / R2 / R3 / R5.
- **DL-TDoA fw 0.5.x (`protocol.md §13.4`, §13.13).** `PRXDLSchedule` reads
  `0x010C` (slot, responders, session, 8-byte hardware id) and seeds
  `PRXDLConfig`/`PRXDLConfigDraft` in place of the caller's memory; `provisionDL`
  verifies slot and responders against it. `PRXDLCodec` (byte 1 of `0x010B`, the
  only byte read) is on `PRXDeviceIdentity.dlCodec`. `0x7D` discard pending
  (`discardPendingDL()`). `PRXDeviceIdentity.isDebugBuild` ignores the stale debug
  bit of DL product builds before 0.5.1; `dlUnitKey` is the hardware id, else
  (session, id). `PRXDLCellFrame` maps the cell frame to the venue (origin +
  rotation; 0 = the old +X east). Gates `.dlSchedule`, `.dlDiscardPending`. The
  simulator defaults to fw 0.5.1; the bench cell runs the FiRa codec, slots 2/3/4.
